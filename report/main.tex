...
\texttt{ecb5855}) used raw OCaml~5 effect handlers and stored suspended
...
To deal with this limitation, the final design (commit \texttt{17adae2})
...
(a) \texttt{Empty}: no party has visited yet (default);
(b) \texttt{Waiter}: a live fibre is parked in the cell;
(c) \texttt{Cancelling}: transient state when a fibre cancellation is in
flight;
(d) \texttt{Cancelled}: the waiter in the cell has been cancelled;
(e) \texttt{Refuse}: the cell refuses to satisfy the next \texttt{resume}
operation;
(f) \texttt{Resumed}: continuation successfully handed off;
(g) \texttt{Taken}: value grabbed by a concurrent suspend;
(h) \texttt{Broken}: a SYNC resume has timed out;
(i) \texttt{Value}: resume deposited a value before suspend;
(j) \texttt{WrappedValue}: as \texttt{Value}, but distinguishable from
\texttt{Waiter}.
...
Initially, when a cell is in \texttt{Empty} state, one of the following
two transitions is possible:
(a) \texttt{Empty} $\to$ \texttt{Value}, when a \texttt{resume} reaches
the cell first; or
(b) \texttt{Empty} $\to$ \texttt{Waiter}, when a \texttt{suspend} reaches
the cell before any \texttt{resume}.

\paragraph{Case~I: \texttt{resume} arrives first.}
The \texttt{value} passed in by the resume is stored into the CQS
cell. Any subsequent store directed at the slot will be instantly
satisfied. Two modes of resumption are supported by
CQS~\cite{koval2023cqs}:
(a) \textbf{\texttt{Async}}: deposits the value into the cell and
returns immediately. The paired \texttt{suspend} will claim it via the
\emph{elimination path}.
(b) \textbf{\texttt{Sync}}: concurrent resumes and stores are still
allowed into the CQS, but the resume busy-waits for a bounded time. If
no concurrent \texttt{suspend} matches within that window, the cell is
marked \texttt{Broken}; otherwise the suspended fibre is immediately
continued with the value passed by the \texttt{resume}.

All six primitives use \texttt{Async} mode because none of them
requires a synchronous handshake: a \texttt{release} or
\texttt{count\_down} can return as soon as the value is enqueued.
\texttt{Sync} mode exists for use cases that require the resumer to
confirm the value was actually claimed before proceeding.

\paragraph{Case~II: \texttt{suspend} arrives first.}
A \texttt{suspend} action either is satisfied by an already-deposited
\texttt{resume} (the cell transitions \texttt{Empty} $\to$
\texttt{Waiter} $\to$ \texttt{Resumed}), or it enters an empty cell and
parks (\texttt{Empty} $\to$ \texttt{Waiter}). Once parked, two further
outcomes are possible:
(a) \emph{Resumption Path}: a matching \texttt{resume} arrives, and the
cell transitions \texttt{Waiter} $\to$ \texttt{Resumed} with the value
supplied by the resumer.
(b) \emph{Cancellation Path}: a \texttt{cancel} request fires on the
parked fibre, transitioning the cell to \texttt{Cancelled} or
\texttt{Refuse} depending on the cancellation mode (see
\cref{sec:impl-cancel}).
...
\texttt{resume} calls.
\paragraph{Phase~1 --- mark \texttt{Cancelling}.}
\texttt{try\_mark\_cancelling} (\texttt{lib/sqs\_effects.ml:422--434})
CAS-es the cell from \texttt{Waiter} to \texttt{Cancelling}. If the
cell is already \texttt{Resumed}, the cancellation loses the race and
returns \texttt{false}: the resume is already committed and the fibre
will receive the value normally.
\paragraph{Phase~2 --- finalise.}
\texttt{handle\_cancellation}
(\texttt{lib/sqs\_effects.ml:460--489}) installs the final state in
the cell. Behaviour here depends on the configured cancellation mode.

CQS supports two modes of cancellation: \textbf{Simple} and
\textbf{Smart}~\cite{koval2023cqs}.

In the \textbf{Simple} mode, when a cell is marked \texttt{Cancelled}
the waiter is discontinued, the next \texttt{resume} reaching the cell
skips it and fails (it does not retry cancelled cells), and the
segment may eventually be unlinked once all of its slots are
\texttt{Cancelled}.

The \textbf{Smart} mode, by contrast, interacts with the primitive by
invoking an \texttt{on\_cancellation()} handler. If this handler
returns \texttt{true}, the cell is marked \texttt{Cancelled}, with an
extra check for any value that may have been inserted while the cell
was in the \texttt{Cancelling} state; such a stranded value is
forwarded with another \texttt{resume} call so that no permits are
lost. If \texttt{on\_cancellation()} returns \texttt{false}, the cell
is marked \texttt{Refuse}: the next \texttt{resume} arriving at this
cell reroutes its value back into the data structure (via
\texttt{try\_return\_refused\_value}) before discontinuing the
continuation.

The \texttt{Cancelling} transient state serialises against
\texttt{Async} resumers: a resumer that finds \texttt{Cancelling} must
CAS against the live \texttt{Cancelling~k} pointer (rather than
reconstructing one) and may deposit a value into it. The cancellation
handler reads this back via \texttt{Atomic.exchange} in
\texttt{mark\_impl}.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.32\linewidth]{mean_latency_T1.png}\hfill
  \includegraphics[width=0.32\linewidth]{mean_latency_T2.png}\hfill
  \includegraphics[width=0.32\linewidth]{mean_latency_T8.png}
  \caption{Mean per-op latency (ns) per primitive at thread counts
  \texttt{T=1, 2, 8} (left to right), grouped by implementation.}
  \label{fig:lat-grid}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.32\linewidth]{throughput_T1.png}\hfill
  \includegraphics[width=0.32\linewidth]{throughput_T2.png}\hfill
  \includegraphics[width=0.32\linewidth]{throughput_T8.png}
  \caption{Throughput (ops/s) per primitive at \texttt{T=1, 2, 8}
  (left to right). \texttt{T=1} is the uncontended fast path;
  \texttt{T=2}, \texttt{T=8} force cross-domain hand-off on the
  contended workloads.}
  \label{fig:tput-grid}
\end{figure}
